...
<img src="examples/characters/sherlock-holmes/assets/portrait.png" alt="Original literary illustration of Sherlock Holmes" width="160" />

**A Visitor at Baker Street** is a featured character built from Arthur Conan Doyle's original stories. Bring a puzzle, ask about Norbury, or spend an evening talking by the fire. His portable pack includes a detailed identity, three scenes, an original portrait, and sixteen source-backed background notes.

Open **Characters → Meet Sherlock Holmes**, choose a parent folder, and select **Create my Holmes**. Then open Chat with your configured local model. The app creates your own editable copy; it refuses to overwrite an existing folder. No source checkout or network access is needed to install the embedded character.

[Character pack and sources](examples/characters/sherlock-holmes/README.md) · [Detailed character guide](examples/characters/sherlock-holmes/CHARACTER_GUIDE.md) · [Memory and retrieval demonstration](examples/characters/sherlock-holmes/DEMO.md)

Try **“What does Norbury mean to you?”**, then open Sources. To demonstrate persistent memory, supply a new fictional detail, verify its Markdown memory, and ask about it in a new chat. Model quality affects both portrayal and automatic extraction; the demonstration guide separates those checks.
...
- Credential-vault failures: unlock or repair the signed-in user's OS credential service, then retry. On Windows this is Credential Manager; macOS uses Keychain; Linux requires a working Secret Service session. tz-chatter does not fall back to a plaintext token. Existing schema-1 settings migrate on first load only after the secure write succeeds.
