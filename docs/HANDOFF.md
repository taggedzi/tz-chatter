...
## 2026-09-14 — T37: CodeQL review-harness code-construction remediation

Scope and outcome: resolved GitHub CodeQL alerts 1 and 2, both medium-severity `js/bad-code-sanitization` findings in documentation-only synthetic browser-review harnesses. `docs/review/ui_review.mjs` previously interpolated message text into a `Runtime.evaluate` expression, while `docs/review/gui_remediation_review.mjs` interpolated button labels. Both now use fixed function declarations with values supplied separately through Chrome DevTools `Runtime.callFunctionOn` arguments. The adjacent legacy button helper in `ui_review.mjs` was converted at the same time.

Files changed: `docs/review/ui_review.mjs`, `docs/review/gui_remediation_review.mjs`, `docs/PLAN.md`, `docs/STATUS.md`, and this handoff. Historical screenshots and result JSON regenerated by the verification run were deliberately restored because the behavior remained valid and those artifacts were not part of the security fix.

Decisions added/superseded: none. This hardens review tooling and does not change shipped application behavior.

Verification and actual results: both Edge/CDP harnesses PASS; the GUI remediation harness reports zero runtime errors and retains all recorded layout/interaction assertions; `node --check` passes for both scripts; `npm run test:unit` PASS (42); frontend lint/typecheck/build PASS; actionlint 1.7.12 PASS; `git diff --check` PASS. Commit `5825a13` was pushed to `main`. Hosted CI run 34896126560 PASS and CodeQL run 34896126562 PASS. GitHub marked alerts 1 and 2 fixed at 2026-09-14T20:59:54Z.
...
## 2026-09-14 — T39: OS-protected provider credentials

Scope and outcome: moved optional provider bearer tokens out of plaintext `provider-settings.json` into the current user's operating-system credential vault through the Rust keyring adapter. Schema 2 stores only `has_bearer_token`, and the load command never returns an existing secret. Rust resolves tokens for provider health/discovery/embeddings, user chat and its background extraction/embedding work, and initiative delivery/scheduling. Automatic lookup requires the configured provider id, kind, and endpoint to match, preventing an unsaved endpoint edit from redirecting a saved token. The Settings password field remains blank for a saved token, offers an explicit Remove saved token action, and shows secure-store/migration failures rather than silently falling back to defaults or overwriting legacy settings.

Schema-1 plaintext settings migrate only after a successful OS credential write. Schema-2 JSON and its migrated replacement do not serialize `bearer_token`. Saving/replacing and removing tokens update the OS store with rollback for partial multi-provider operations. Duplicate provider IDs are rejected. Rust debug formatting redacts token values. ADR-019 records the native-store boundary and at-rest security limits.

Files changed: `src-tauri/Cargo.toml`, `src-tauri/Cargo.lock`, `src-tauri/src/providers.rs`, `src-tauri/src/lib.rs`, `src/providers.ts`, `src/ProviderPanel.tsx`, `src/ConversationPanel.tsx`, `src/CharacterPanel.tsx`, `README.md`, `docs/PROJECT.md`, `docs/PLAN.md`, `docs/DECISIONS.md`, `docs/STATUS.md`, and this handoff.

Verification and actual results: `cargo test --locked` PASS (179 passed, 3 ignored including the intentionally opt-in OS test); focused provider tests PASS; `cargo test --locked live_os_credential_store_round_trip -- --ignored` PASS on Windows using a unique temporary entry that was deleted and verified absent; Rust fmt and strict Clippy PASS; `npm run test:unit` PASS (47), frontend lint/typecheck/build PASS; `cargo audit --file src-tauri/Cargo.lock` PASS (0 vulnerabilities; 7 pre-existing allowed upstream warnings); `git diff --check` PASS. Deterministic tests cover no JSON secret, safe/failed legacy migration, preserved and removed credentials, failed removal metadata recovery, multi-provider write rollback, duplicate IDs, endpoint-bound resolution, and missing-secret errors.

Limitations: live Windows Credential Manager storage is verified, but no packaged native Settings click-through or authenticated provider request with a saved test token was run. Linux Secret Service and macOS Keychain are selected by the cross-platform crate but have not been exercised on those hosts. Protection is at rest; a compromised signed-in OS account or running application can still access a usable credential. Migration does not erase unrelated historical plaintext copies in backups, snapshots, or filesystem history; users with previously saved tokens should rotate them if those copies might have escaped.

Next concrete action: review and commit T39, then test packaged Settings token save → restart → authenticated provider operation → explicit removal on Windows. Treat T38's hosted dry-run and the T16/T18 release gate independently; public publication remains on hold.


## 2026-09-15 — T40: glib iterator safety backport

Scope and outcome: implemented the user's requested fix for Dependabot alert 1 (RUSTSEC-2024-0429 / GHSA-wrw7-89jp-8q8g). Linux resolves glib 0.18.5 through Tauri/GTK; Windows does not include it. GTK dependencies require the 0.18 series, so the fixed 0.20 series cannot simply replace it. The vendored published 0.18.5 crate contains exactly upstream gtk-rs-core PR 1343's two-line mutable output-pointer correction. All 121 upstream files, MIT license, copyright notice, and source commit metadata are retained. The original archive checksum was verified against Cargo.lock before extraction; a per-file checksum manifest and Node test verify the exact allowed delta. The version is unchanged and no audit ignore was added.

Cargo's root patch selects the vendored source. A test-only workspace member shares the application lockfile/patch; the application remains the default member. CI and release validation run this member in release mode. `vendor/README.md` covers provenance, verification, scanner limitations, and removal: wait for a fixed compatible upstream release (or a compatible GTK/Tauri stack update), remove the override, update Cargo, remove the vendored files/integrity test, and keep the optimized regression. A newer incompatible glib direct dependency does not replace the vulnerable one.

Files changed: Cargo manifest/lock, `vendor/`, `src-tauri/security-tests/glib-variant/`, `tests/glibBackport.test.mjs`, CI/release workflows, README, PLAN, STATUS, and HANDOFF. No product architecture decision changed; no ADR added.

Verification: Ubuntu 22.04.5 WSL, Rust 1.94.1, GLib 2.72.4: `CARGO_TARGET_DIR=/tmp/tz-chatter-t40-target cargo test --locked --release --manifest-path src-tauri/Cargo.toml -p glib-variant-regression` PASS (2 tests). An isolated copy of the original checksum-verified crate using the identical regression and release settings reproduces SIGSEGV (signal 11). Coverage includes all five affected iterator methods, forward/reverse collection, empty and UTF-8 strings, and exhaustion. Cargo metadata confirms one local glib selected for Linux and the application as sole default workspace member. Windows `cargo test --locked --manifest-path src-tauri/Cargo.toml` PASS (179, 3 existing ignored), fmt and strict application Clippy PASS; Linux regression Clippy PASS (34 existing upstream glib compiler warnings preserved). `npm run test:unit` PASS (49); ESLint PASS after fixing explicit Node URL/Buffer imports; integrity tests PASS; checksum-verified actionlint 1.7.12 PASS on both changed workflows; audit reports 0 vulnerabilities and 6 pre-existing unmaintained warnings; diff check PASS. No dependency versions changed beyond switching glib source and adding the test member.

Environment and temporary work: installed missing GLib development headers in existing Ubuntu WSL. Original-crate control is ignored under `src-tauri/target/t40-unpatched`, actionlint under `src-tauri/target/t40-tools`, and WSL logs/target under `/tmp/tz-chatter-t40-*`. The initial control-run wrapper lost its shell status variable; a corrected wrapper explicitly confirmed SIGSEGV and returned success only for that expected failure.
...
## 2026-09-15 — T42: Sherlock Holmes, A Visitor at Baker Street

Scope and outcome: delivered the explicitly requested front-facing literary character. The portable pack under examples/characters/sherlock-holmes contains 23 playable files: core character sheet, conditional first-contact/returning persona, default scene, three reusable locations, sixteen source-backed background records, and an original generated portrait. Guides include a detailed editorial characterization, ten original dialogue studies, named Doyle source register, exact image-generation prompt, and a repeatable demonstration covering retrieval, automatic extraction, new-session recall, correction, scenes, initiative, and export. No invented user history or transcript is seeded.

Integration: src-tauri/src/featured.rs embeds the pack at compile time and installs through the existing validated importer into a new child of the user-selected parent. Existing folders are refused. Failed library registration preserves the valid installed copy and explains recovery through Add. lib.rs and src/characters.ts expose one typed command; CharacterPanel.tsx/App.css feature Holmes with portrait, folder picker, install action, and compact demo instructions. No dependency, schema, provider configuration, automatic download, or release workflow changed. ADR-020 and PROJECT document the accepted sample boundary.

Verification: 184 Rust library tests PASS, 4 ignored; 49 frontend unit tests PASS; Rust fmt and strict Clippy all-targets PASS; ESLint/TypeScript/Vite PASS; local guide links and diff checks PASS. Five new integration scenarios cover byte-for-byte installed/source identity, all real vault parsers, source citation in retrieval chunks, all scene budgets, export/import, conflicts, registration failure, rebuild/reopen and isolation. Final default input estimates including retrieved notes are 2600–2918 of a 3328 input allowance. The synthetic-IPC UI harness passes at 1280x800 and 900x620 with loaded portrait and no overflow/runtime errors; native picker and IPC clicks are not verified by that harness.

Live models: qwen2.5:7b initially saved and recalled a fresh token but called the visitor Watson. A revised run failed to save the token. llama3.1:8b saved/retrieved it but denied knowing it and invented Mycroft involvement at Norbury. First-contact defaults were made conditional on absent history, with matching scene continuity wording. The ensuing Qwen continuity run and final pack run both passed actual ConversationService → extraction worker → sourced Markdown → disk reopen/new service → new-session exact recall. Last run used copper-28b497c7 and no manual memory insertion. These are small scoped runs, not a reliability rate. All five JSON reports and a candid summary are preserved under docs/review/holmes-*.

Environment: normal shell, node, image reader, and browser startup intermittently failed in sandbox setup; approved repository commands and the existing headless browser approach worked. Full cargo test hit Windows error 5 while trying to replace the user's running debug executable. Integration scenarios were moved into the library test target, retaining coverage without stopping the app. The portrait came from the built-in image_gen tool and was copied into the project; ART.md records its exact prompt. No refresh/reset credit was needed or consumed.

Remaining limits: some model replies still ask too many questions, use an unchosen gendered address, or make a memory promise before storage. Live correction, hybrid embeddings, initiative/tray/notifications, packaged native installation and real desktop restart remain unrun. Public release is still on hold. No commit or push was made.

Next executable step: rebuild/relaunch when the running development app can release its executable, open Characters → Meet Sherlock Holmes, create a personal copy, and follow examples/characters/sherlock-holmes/DEMO.md. Reproduce the opt-in live check with cargo test --locked --manifest-path src-tauri/Cargo.toml --lib holmes_live_chat_extraction_and_new_session_recall -- --ignored --nocapture. Then continue the independent T39 native Settings and T16/T18 release evidence, with T38 hosted dry-run separate.
